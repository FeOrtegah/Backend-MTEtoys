...
export const markAsPreparing = async (req, res) => {
  try {
    const pedido = await Order.findById(req.params.id);
    ...
    if (pedido.estado !== "pagado") {
      return res.status(400).json({
        message: "Solo se pueden marcar como en preparación los pedidos pagados",
      });
    }
    ...
    console.error("Error marcando pedido como en preparación:", error);
    return res.status(500).json({ message: "Error al marcar el pedido como en preparación" });
  }
};
...
export const markAsShipped = async (req, res) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({
        message: "Solo se pueden marcar como enviados los pedidos pagados o en preparación",
      });
    }
